# 👋 Hi, I'm Alfred

💻 IT Student | Developer
📍 Norway

I enjoy working with **web development, Linux systems, and backend development**.
Currently learning more about **software development, servers, and modern web frameworks**.

---

# 🚀 About Me

* 🎓 VG2 Information Technology student
* 🖥 Interested in **backend, frontend, and system administration**
* 🐧 Linux user
* ⚙️ I enjoy building projects and experimenting with new technologies

---

# 🛠 Tech Stack

### Languages

* HTML
* CSS
* JavaScript
* SQL
* Node.js and express
* next.js og react
* python
* 

### Tools

* Git
* Linux
* VS Code

---

# 📂 Projects

Here are some projects from my repositories:

### 🌐 Web Development Projects

* Web applications and frontend experiments
* Backend APIs and server-side experiments

### ⚙️ System / Linux Projects

* Server configuration
* Linux setup and experimentation

More projects can be found in my repositories.

---

# 📊 GitHub Stats

![Stats](https://github-readme-stats.vercel.app/api?username=alfreditann\&show_icons=true\&theme=tokyonight)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=alfreditann\&layout=compact\&theme=tokyonight)

---

# 🔗 Links

* GitHub: https://github.com/alfreditann

---

⭐ If you like my projects, feel free to check them out!
